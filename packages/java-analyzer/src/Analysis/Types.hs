...
-- Adapted from core-v3/Analysis.Types for Java-specific analysis.
...
data DeferredKind
  ...
  | TypeResolve           -- ^ type reference resolution (extends, implements, field types)
  | CallResolve           -- ^ method/constructor call resolution
  | InheritanceResolve    -- ^ extends/implements resolution
  | AnnotationResolve     -- ^ annotation type resolution
  deriving (Show, Eq)
...
-- ── Scope Types (Java-specific) ─────────────────────────────────────────

data ScopeKind
  = PackageScope          -- ^ package-level scope
  | ClassScope            -- ^ class body
  | InterfaceScope        -- ^ interface body
  | EnumScope             -- ^ enum body
  | RecordScope           -- ^ record body
  | MethodScope           -- ^ method body
  | ConstructorScope      -- ^ constructor body
  | LambdaScope           -- ^ lambda expression body
  | BlockScope            -- ^ block statement
  | StaticInitScope       -- ^ static initializer
  | InstanceInitScope     -- ^ instance initializer
  | TryCatchScope         -- ^ try-catch block
  | SwitchScope           -- ^ switch statement/expression
  | LoopScope             -- ^ for/while/do-while body
  | SynchronizedScope     -- ^ synchronized block
  | ModuleScope           -- ^ top-level module/file scope
  deriving (Show, Eq)

data DeclKind
  = DeclClass             -- ^ class declaration
  | DeclInterface         -- ^ interface declaration
  | DeclEnum              -- ^ enum declaration
  | DeclRecord            -- ^ record declaration
  | DeclAnnotationType    -- ^ annotation type declaration
  | DeclMethod            -- ^ method declaration
  | DeclConstructor       -- ^ constructor declaration
  | DeclField             -- ^ field declaration
  | DeclParameter         -- ^ method/constructor parameter
  | DeclLocalVariable     -- ^ local variable
  | DeclEnumConstant      -- ^ enum constant
